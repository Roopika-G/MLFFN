...
def generate_ffn_lexicon(ffn_model, embs, vocab_limit=10000):
    """
    Generate high_empathy/no_empathy lexicon using the trained FFN model.
    """
    ...
    df = pd.DataFrame(scores, columns=["word", "high_empathy_Score"])
    df = df.sort_values(by="high_empathy_Score", ascending=False)
...
def generate_ridge_lexicon(ridge_model, embs, vocab_limit=10000):
    """
    Generate high_empathy/no_empathy lexicon using Ridge regression coefficients.
    """
...
def save_lexicon_files(df, model_name, base_dir="data"):
    ...
    # Save top words
    top_high_empathy_file = f"{base_dir}/{model_name}_top_high_empathy_words.csv"
    top_no_empathy_file = f"{base_dir}/{model_name}_top_no_empathy_words.csv"
    
    df.head(50).to_csv(top_high_empathy_file, index=False)
    df.tail(50).to_csv(top_no_empathy_file, index=False)
    
    print(f"📁 Saved {model_name} lexicon files:")
    print(f"   - {full_file}")
    print(f"   - {top_high_empathy_file}")
    print(f"   - {top_no_empathy_file}")
...
def create_visualizations(df, model_name, base_dir="data"):
    ...
    plt.hist(df["high_empathy_Score"], bins=50, color="skyblue", edgecolor="black", alpha=0.7)
    plt.title(f"{model_name.upper()} Model: Distribution of high_empathy-no_empathy Scores")
    plt.xlabel("high_empathy Score (0=no_empathy, 1=high_empathy)")
    ...
    # Plot top high_empathy words (horizontal bar chart)
    plt.figure(figsize=(12, 8))
    top_high_empathy = df.head(20)
    colors = plt.cm.Greens(np.linspace(0.4, 0.8, len(top_high_empathy)))
    plt.barh(range(len(top_high_empathy)), top_high_empathy["high_empathy_Score"], color=colors)
    plt.yticks(range(len(top_high_empathy)), top_high_empathy["word"])
    plt.gca().invert_yaxis()
    plt.title(f"{model_name.upper()} Model: Top 20 high_empathy Words")
    plt.xlabel("high_empathy Score")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{base_dir}/{model_name}_top_high_empathy_words.png", dpi=300, bbox_inches='tight')
    plt.close()

    # Plot top no_empathy words (horizontal bar chart)
    plt.figure(figsize=(12, 8))
    top_no_empathy = df.tail(20)
    colors = plt.cm.Reds(np.linspace(0.4, 0.8, len(top_no_empathy)))
    plt.barh(range(len(top_no_empathy)), top_no_empathy["high_empathy_Score"], color=colors)
    plt.yticks(range(len(top_no_empathy)), top_no_empathy["word"])
    plt.gca().invert_yaxis()
    plt.title(f"{model_name.upper()} Model: Top 20 no_empathy Words")
    plt.xlabel("high_empathy Score")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{base_dir}/{model_name}_top_no_empathy_words.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def create_comparison_visualization(lexicons, base_dir="data"):
    ...
    # Compare top 10 high_empathy words across models
    for i, (model_name, df) in enumerate(lexicons.items()):
        top_high_empathy = df.head(10)
        plt.subplot(2, 2, i+1)
        bars = plt.barh(range(len(top_high_empathy)), top_high_empathy["high_empathy_Score"], color=colors[i], alpha=0.7)
        plt.yticks(range(len(top_high_empathy)), top_high_empathy["word"])
        plt.gca().invert_yaxis()
        plt.title(f"{model_name.upper()}: Top 10 high_empathy Words")
        plt.xlabel("high_empathy Score")
        plt.grid(True, alpha=0.3)
    
    # Overall comparison
    plt.subplot(2, 2, 3)
    model_means = [df["high_empathy_Score"].mean() for df in lexicons.values()]
    model_stds = [df["high_empathy_Score"].std() for df in lexicons.values()]
    
    bars = plt.bar(models, model_means, yerr=model_stds, color=colors, alpha=0.7, capsize=5)
    plt.title("Model Comparison: Average high_empathy Scores")
    plt.ylabel("Average high_empathy Score")
    ...
    if len(lexicons) == 2:
        ...
        plt.title(f"Top 100 high_empathy Words Overlap\n{model_names[0]} vs {model_names[1]}")
...
def main():
    ...
    print("🚀 Generating high_empathy/no_empathy Lexicons from FFN and Ridge Models")
    print("=" * 60)
    ...
    for model in ["ffn", "ridge"]:
        ...
        print(f"   - {model}_top_high_empathy_words.csv") 
        print(f"   - {model}_top_no_empathy_words.csv")
        print(f"   - {model}_lexicon_distribution.png")
        print(f"   - {model}_top_high_empathy_words.png")
        print(f"   - {model}_top_no_empathy_words.png")
    
    print(f"\n📈 Comparison:")
    print(f"   - models_comparison.png")
    
    print(f"\n💡 Insights:")
    print(f"   - FFN average high_empathy score: {ffn_lexicon['high_empathy_Score'].mean():.4f}")
    print(f"   - Ridge average high_empathy score: {ridge_lexicon['high_empathy_Score'].mean():.4f}")
